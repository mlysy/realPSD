...
suppressMessages(require(nlsr)) # for NLS optimization
...
for(ii in 1:length(Q_vec)) {
  ...
  # --------- optimization -----------
  start <- phi_list[[ii]]
  names(start) <- c("f0", "gamma", "Rw")
  # try to set the initial gamma to be true value (as MATLAB code)
  start[2] <- f0*Q 
  # first, fix f0 and Rw, optimize gamma (i.e. to optimize Q since f0 is fixed)
  opt1 <- nlsr::nlfb(start = start, resfn = obj$fn,
    lower = 0,
    maskdix = c(1,0,1), # indices of parameters to be fixed
    weights = rep(1,3)
  )
  # second, optimize f0 and Q
  opt2 <- nlsr::nlfb(start = opt1$coefficients,
                     resfn = obj$fn,
                     lower = 0,
                     maskdix = c(0,0,1),
                     weights = rep(1,3))
  # third, optimize all three parameters
  ...
  # ---------- check optim ----------
  optim_proj(fun = obj$fn, xsol = opt3$coefficients, maximize = FALSE,
    xnames = c("f0", "gamma", "Rw"))
  # ---------- check tau_hat ----------
  # phi_hat <- opt$par
