...
suppressMessages(require(minqa)) # for NLS optimization
...
Many other nonlinear optimization routines have been tried, the derivative-free optimization by quadratic approximation supplied by `minqa::bobyqa` gives the best estimation result, as shown by Figure \ref{fig:check_nls_optim1} ~ \ref{fig:check_nls_optim4}.
...
for(ii in 1:length(Q_vec)) {
  ...
  obj <- TMB::MakeADFun(data = list(model_name = "SHOWFit",
                          ...
                          silent = TRUE, DLL = "realPSD_TMBExports")
  # --------- optim BGFS -----------
  opt <- minqa::bobyqa(par = phi_list[[ii]], fn = obj$fn,
    lower = rep(0,3),
...
The estimatation of $\sigma^2$ when $Q = 100, 500$ may have significantly larger errors (larger than 1% but less than 5%). But the estimated parameters are still quite close to the true ones.
...
The NLS method cannot always give the best estimation as MLE did. But by using `minqa::bobyqa`, we can still get good enough results.
