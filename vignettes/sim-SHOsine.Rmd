...
fseq <- seq(from = f0-f0/sqrt(2), to = f0+f0/sqrt(2), by = 0.01)
N <- length(fseq)
psd <- psdSHO(fseq, f0, Q, k, Kb, Temp, unit_conversion) + Aw
sin_fft <- fft_sin(fseq, f0, Q, fs, unit_conversion)
...
sol <- uniroot(fisherEq, c(0,M), 
  extendInt = "yes", 
  q = N, logSort = TRUE, pval = pval)
psd_line <- sol$root * psd * sum(gstat)
log10(max(psd_line))
plot(x = fseq/10^3, y = Y/fs, xlim = c((f0-1500)/10^3, (f0+1500)/10^3), 
...
# psd_sin <- sin_fft * Conj(sin_fft)
# psd_sin <- Re(psd_sin)
# plot(x = fseq[idx]/10^3, y = psd_sin[idx]/fs, type = "l", lwd = 0.3, log = "xy")
# psd_y <- sim_cnorm * sqrt(2*pi*fs*psd)
# psd_y <- psd_y * Conj(psd_y)
# psd_y <- Re(psd_y)
# plot(x = fseq[idx]/10^3, y = psd_y[idx]/fs, type = "l", lwd = 0.3, log = "xy")
```
