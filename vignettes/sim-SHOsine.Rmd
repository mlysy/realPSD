...
Q <- 100
fseq <- seq(from = 32e3, to = 35e3, by = 1/Time)
N <- length(fseq)
fseq <- fseq / 10^3
psd <- psdSHO(fseq * 10^3, f0, Q, k, Kb, Temp, unit_conversion) + Aw
sin_fft <- fft_sin(fseq, f0, Q, unit_conversion)
x1 <- rnorm(N, 0, sqrt(1/2)) * sqrt(psd * fs)
x2 <- rnorm(N, 0, sqrt(1/2)) * sqrt(psd * fs)
Y <- complex(real = x1, imaginary = x2)
Y <- Re(Y)
plot(x = fseq, y = Y, xlab = "frequency (kHz)", ylab = expression(paste("PSD (", fm^2/Hz, ")")),
      type = "l", lwd = 0.3, log = "xy")
lines(x = fseq, y = psdSHO(fseq * 10^3, f0, Q, k, Kb, Temp, unit_conversion), 
      col = "red")
legend("topright",
