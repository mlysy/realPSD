...
simNum <- 10            # Number of simulations to run (per each Q_s)
# random_seed = rng('shuffle'); # Seeds the random number generator based on the current time
random_seed <- 123
model_type <- 'SHOW'    # SHOW or SHOWF
binfit_size <- 100      # size of bins for fitting
T_s <- 5                # Total time
SF_s <- 1e7             # Sampling frequency
f0_s <- 33553           # Resonance frequency, Hz
Q_s  <- 500             # Quality factor
alpha_s <- 0.55         # 1/f decay exponent
k_s  <- 0.172           # Cantilever stiffness, N/m
