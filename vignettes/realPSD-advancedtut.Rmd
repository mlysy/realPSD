...
This is a workaround to let the TMB object pointer find the current model object (which is not built into the **realPSD** package). Here we specifically used one line `UFun<type>(f)` as the return. In this way, the memory will be allocated only once, and the C++ compiler knows to move instead of copying the object. 

If users want to get additional data, they can modify `make_Ufun` as follows:
...
where we returned the overloaded constructor.
