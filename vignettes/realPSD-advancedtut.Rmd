...
**realPSD** provides three very important base classes: 

  - `psd_model` for any parametric PSD model
  - `show_model` for any SHOW model (see the example below)
  - `shof_model` for any SHO model with $1/f$ noise (see the example below)

One can easily create a model object by using the `new()` method in the above R6 base classes. For more details, please see the documentation for each class.
