...
namespace realPSD {
  ...
  template <class Type>
  inline void LP<Type>::set_Zbar(cRefMatrix<Type>& Zbar) {
  ...
  template <class Type>
  inline Type LP<Type>::zeta(cRefMatrix<Type>& Ubar) {
  ...
  template <class Type>
  inline Type LP<Type>::nll(cRefMatrix<Type>& Ubar, const Type zeta) {
    // return logUbar(0,0);
    // logUbar_ = Ubar.array().log();
    // ZLU_ = Zbar_ - logUbar_;
    set_ZLU(Ubar);
  ...
  /// @param[in] Ubar Vector of bin-averaged normalized PSDs.
  template <class Type>
  inline void LP<Type>::set_ZLU(cRefMatrix<Type>& Ubar) {
    logUbar_ = Ubar.array().log();
    ZLU_ = Zbar_ - logUbar_;
    return;
  }
  ...
  /// @param[in] zeta Log of the PSD scale factor, `zeta = log(sigma^2)`.
  template <class Type>
  inline void LP<Type>::res(RefMatrix<Type> R, cRefMatrix<Type>& Ubar,
			    const Type zeta) {
    // zeta_ = zeta(Ubar);
    set_ZLU(Ubar);
    R = ZLU_.array() - zeta;
  ...
  /// @return Value of the objective function (scalar).
  template <class Type>
  inline Type LP<Type>::nlp(cRefMatrix<Type>& Ubar) {
    // logUbar_ = Ubar.array().log();
    zeta_ = zeta(Ubar);
    return nll(zeta_);
    // ZLU_.array() -= zeta_;
