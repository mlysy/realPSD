...
#' @param par SHOW parameters `(k, f0, Q, Sw)` or `(k, f0, Q, Sw, Af, alpha)`.
...
#' @export
get_phi <- function(par, method = c("MLE", "NLS", "LP"), model = c("SHOW", "SHOWF"), Temp, const) {
  Kb <- 1.381e-23
  k <- par[1]
  f0 <- par[2]
  Q <- par[3]
  ...
  if(model == "SHOW") {
    ...
    if(method == "MLE") {
      tau <- tau/const
      phi_zeta <- setNames(c(log(f0), log(Q), log(Rw), log(Rf), log(alpha), tau), nm=c("f0", "Q", "Rw", "Rf", "alpha", "tau"))
    } else if (method == "NLS") {
      tau <- tau/const
      phi_zeta <- setNames(c(log(f0), log(Q), log(Rw), log(Rf), log(alpha), tau), nm=c("f0", "Q", "Rw", "Rf", "alpha", "tau"))
    } else if (method == "LP") {
      zeta <- log(tau) - const
      phi_zeta <- setNames(c(log(f0), log(Q), log(Rw), log(Rf), log(alpha), zeta), nm=c("f0", "Q", "Rw", "Rf", "alpha", "tau"))
    }
  }
  return(phi_zeta)
}
