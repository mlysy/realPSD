...
#' @param get_jac If TRUE, return the numerical Jacobian of the SHOW parameters k, f0, Q, Af, only valid if method == "NLS".
...
#' 
#' @export
shof_fit <- function(fseq, Ypsd, fs, Temp,
                     method = c("lp", "nls", "mle"),
                     bin_size, bin_type, 
                     phi0, Sw0, tau0,
  ...
  if(method == "lp") {
    out <- shof_fit_lp(fseq = fseq, Ypsd = Ypsd, fs = fs, Temp = Temp,
                       bin_size = bin_size, bin_type = bin_type,
                       phi0 = phi0, Sw0 = Sw0, tau0 = tau0,
                       fit_type = fit_type, optimizer = optimizer,
                       vcov = vcov, ...)
  } else if(method == "nls") {
    out <- shof_fit_nls(fseq = fseq, Ypsd = Ypsd, fs = fs, Temp = Temp,
                        bin_size = bin_size, bin_type = bin_type,
                        phi0 = phi0, Sw0 = Sw0, tau0 = tau0, 
                        fit_type = fit_type, optimizer = optimizer,
                        vcov = vcov, get_jac = get_jac, ...)
  } else if(method == "mle") {
    out <- shof_fit_mle(fseq = fseq, Ypsd = Ypsd, fs = fs, Temp = Temp,
                        phi0 = phi0, Sw0 = Sw0, tau0 = tau0,
                        fit_type = fit_type, optimizer = optimizer,
                        vcov = vcov, ...)
  }
