...
To provide efficient estimation for general parametric PSD models, **realPSD** requires you to write your own model in a standalone C++ header file (with extension `.hpp`). Here we have it ready for you. 
...
Basically, we defined a `UFun` class within our own namespace `ou` to avoid any potential scope conflict with the **realPSD** package. Later whenever we refer to this class, we will use `ou::UFun` to emphasize its namespace. The `ou::UFun` class is nothing fancy but just a collection of necessary data and functions we need to evaluate the PSD of the OU process. In our specific example, we need to provide 
  ...
  - the length of the simulated time series `N` which is supplied via the constructor function `UFun(int N)`
  - a vector of frequencies `f` which is supplied via function `set_f`
  - a function `eval` to evaluate the PSD. 

If you look at the definition of `eval` function
...
Here `classname` is user-defined and we call it "ou_model". It will be an object inherit from an existing built-in R6 class object `psd_model` provided by **realPSD**, so we set `inherit = realPSD::psd_model`. `tmb_model_` should be the same as the `model` input in `make_psd_model` and `tmb_DLL_` should be the same as the name of the automatically generated C++ file in the above **Step 1**, i.e. "OU_FitMethods" in our case. We can add public methods specifically for the OU model as shown above (i.e. `to_theta`, `to_phi`). Please note that `to_theta` should be treated as a reserved keyword for converting parameters from their computational basis ($\log$ transformed parameterization $\phi$ and $\zeta$) back to the inferential basis (natural parameterization $\theta$). If one specify `to_theta = TRUE` in method `vcov()` for the object model, then it will try to find a method called `to_theta` when converting the standard error estimates by using the sandwich estimator.
...
```{r public_list_psd_class, eval = FALSE}
...
Note that we used R6 class to initialize the OU model object by using the method `ou_model$new()` . The input arguments can be found in the `initialize` method listed above for the R6 base class `realPSD::psd_model`.
...
Before we start to fit the OU PSD, let's first define some helper functions to help us convert parameters.
